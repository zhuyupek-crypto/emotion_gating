...
if _LOCAL_JQ_ROOT not in sys.path:
    sys.path.insert(0, _LOCAL_JQ_ROOT)
...
from jqdata import *
# === PHASE1B_ATTRIBUTION_PRELUDE_BEGIN ===
import functools as _attr_functools
try:
    from rebuild_from_archive.attribution.observer import get_current_observer as _attr_get_current_observer
except Exception:
    from attribution.observer import get_current_observer as _attr_get_current_observer

_attr_original_run_daily = run_daily
_attr_original_order = order
_attr_original_order_value = order_value
_attr_original_order_target = order_target
_attr_original_order_target_value = order_target_value


def _attr_observer():
    return _attr_get_current_observer()


def _attr_wrap_handler(func, time_str):
    @_attr_functools.wraps(func)
    def _attr_wrapped(context, *args, **kwargs):
        obs = _attr_observer()
        obs.set_current_handler(func.__name__, time_str, context)
        obs.snapshot_handler(context, g, func.__name__, time_str, "BEFORE")
        try:
            result = func(context, *args, **kwargs)
            if func.__name__ == "prepare_all":
                obs.observe_after_prepare(context, g)
            return result
        finally:
            obs.snapshot_handler(context, g, func.__name__, time_str, "AFTER")
            obs.clear_current_handler()
    return _attr_wrapped


def run_daily(func, time="09:30"):
    return _attr_original_run_daily(_attr_wrap_handler(func, time), time)


def order_value(security, value, style=None):
    obs = _attr_observer()
    token = obs.emit_order_intent(context, g, "order_value", security, requested_value=value, style=style)
    out = _attr_original_order_value(security, value, style)
    obs.bind_order(token, out)
    return out


def order(security, amount, style=None):
    obs = _attr_observer()
    token = obs.emit_order_intent(context, g, "order", security, requested_amount=amount, style=style)
    out = _attr_original_order(security, amount, style)
    obs.bind_order(token, out)
    return out
...
def order_target(security, amount, style=None):
    obs = _attr_observer()
    token = obs.emit_order_intent(context, g, "order_target", security, requested_amount=amount, style=style)
    out = _attr_original_order_target(security, amount, style)
    obs.bind_order(token, out)
    return out
...
def order_target_value(security, value, style=None):
    obs = _attr_observer()
    token = obs.emit_order_intent(context, g, "order_target_value", security, requested_value=value, style=style)
    out = _attr_original_order_target_value(security, value, style)
    obs.bind_order(token, out)
    return out
# === PHASE1B_ATTRIBUTION_PRELUDE_END ===
import numpy as np
import pandas as pd
...
def buy_v227_一进二(context):
    ...
    for item in g.yjj_candidates:
        ...
        if open_pct < -1e-6 or open_pct > open_hi:
            continue
...
# === PHASE1B_ATTRIBUTION_BUY_OVERRIDES_BEGIN ===

def _attr_sid(context, branch, code):
    return _attr_observer().signal_id_for(context, g, branch, code)


def _attr_branch_sids(context, branch):
    return _attr_observer().signals_for_branch_date(context, branch)


def _attr_mark_handler(context, branch):
    _attr_observer().mark_handler_reached(context, branch)


def _attr_bulk_terminal(context, branch, terminal, reason, stage="ENVIRONMENT_GATE", detail=None):
    obs = _attr_observer()
    for sid in _attr_branch_sids(context, branch):
        if obs.signals_by_id.get(sid, {}).get("terminal_state") == "UNRESOLVED":
            obs.emit_block(sid, context, g, terminal, reason, stage=stage, detail=detail or reason)


def _attr_remaining_sids(context, branch, candidates, start_idx):
    obs = _attr_observer()
    out = []
    for item in list(candidates)[start_idx:]:
        code = item[0] if isinstance(item, tuple) else item
        out.append(obs.signal_id_for(context, g, branch, code))
    return out


def _attr_position_block(signal_id, context, stock):
    pos = context.portfolio.positions.get(stock)
    owner = getattr(g, 'owner', {}).get(stock) if hasattr(g, 'owner') else None
    detail = 'existing position owner=%s amount=%s' % (owner, getattr(pos, 'total_amount', None))
    _attr_observer().emit_block(signal_id, context, g, 'POSITION_BLOCKED', 'DUPLICATE_POSITION', stage='GLOBAL_RESOURCE', detail=detail)


def buy_auction_yiqian(context):
    """Phase1B observed copy of v195 auction sleeve buy."""
    obs = _attr_observer(); branch = 'Auction'; handler = 'buy_auction_yiqian'; _attr_mark_handler(context, branch)
    if not g.enable_auction_yiqian:
        _attr_bulk_terminal(context, branch, 'ROUTED_OUT', 'ROUTE_DISABLED', stage='ROUTE_GATE')
        return
    if not g.auction_yiqian_candidates:
        return
    held = _held_count_by('auction', context)
    slots = max(0, g.auction_yiqian_slots - held)
    if slots <= 0:
        _attr_bulk_terminal(context, branch, 'SLOT_BLOCKED', 'BRANCH_SLOT_FULL', stage='BRANCH_SLOT', detail='held=%s slots=%s' % (held, g.auction_yiqian_slots))
        return
    if context.portfolio.available_cash / context.portfolio.total_value <= 0.3:
        _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'AUCTION_CASH_RATIO_BLOCK', stage='GLOBAL_RESOURCE')
        return
    scale = _win_scale()
    if scale == 0.0:
        _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'WIN_RATE_SCALE_ZERO', stage='COOLDOWN_GATE')
        return
    ...
    cand_list = []
    for s in g.auction_yiqian_candidates:
        sid = _attr_sid(context, branch, s); obs.mark_candidate_loop_reached(sid)
        if s in context.portfolio.positions and context.portfolio.positions[s].total_amount > 0:
            _attr_position_block(sid, context, s); continue
        d = cd[s]
        if d.paused:
            obs.emit_block(sid, context, g, 'DATA_INVALID', 'PAUSED'); continue
        if d.last_price >= d.high_limit * 0.999:
            obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'LIMIT_UP_UNBUYABLE'); continue
        if d.last_price <= d.low_limit * 1.001:
            obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'LIMIT_DOWN_UNBUYABLE'); continue
        obs.mark_handler_eligible(sid, True); obs.mark_branch_eligible(sid, True)
        cand_list.append(s)

    if not cand_list:
        return
    try:
        au_dict = get_call_auction(cand_list, start_date=start, end_date=end, fields=['time', 'volume', 'current'])
    except Exception:
        au_dict = {}
    try:
        df_val = get_valuation(cand_list, start_date=context.previous_date, end_date=context.previous_date, fields=['market_cap', 'circulating_market_cap'])
        val_map = {row['code']: (row['market_cap'], row['circulating_market_cap']) for _, row in df_val.iterrows()} if df_val is not None and not df_val.empty else {}
    except Exception:
        val_map = {}

    qualified = []
    for s in cand_list:
        sid = _attr_sid(context, branch, s)
        d = cd[s]
        kind = g.auction_yiqian_kind.get(s)
        try:
            prev_volume = g.auction_yiqian_prev_volume.get(s, 0)
            if prev_volume <= 0:
                obs.emit_block(sid, context, g, 'DATA_INVALID', 'DATA_MISSING', detail='prev_volume<=0'); continue
            val = val_map.get(s)
            if not val:
                obs.emit_block(sid, context, g, 'DATA_INVALID', 'DATA_MISSING', detail='valuation missing'); continue
            market_cap, circ_cap = val
            if market_cap < 70 or circ_cap > 520:
                obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'AUCTION_MARKET_CAP_FILTER'); continue
            au = au_dict.get(s)
            if au is None or au.empty:
                obs.emit_block(sid, context, g, 'DATA_INVALID', 'DATA_MISSING', detail='auction missing'); continue
            auction_price = au['current'].iloc[0] if 'current' in au.columns else 0
            if auction_price <= 0 or d.high_limit <= 0:
                obs.emit_block(sid, context, g, 'DATA_INVALID', 'OPEN_PRICE_INVALID'); continue
            prev_close_est = d.high_limit / 1.1
            auction_ratio = auction_price / prev_close_est if prev_close_est > 0 else 0
            vol_ratio = au['volume'].iloc[0] / prev_volume if prev_volume > 0 and 'volume' in au.columns else 0
            if vol_ratio < 0.03:
                obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'AUCTION_VOLUME_RATIO_FILTER'); continue
            if kind == 'y2':
                if auction_ratio <= 1.0 or auction_ratio >= 1.06:
                    obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'AUCTION_PRICE_RANGE_FILTER'); continue
            else:
                if auction_ratio <= 0.98 or auction_ratio >= 1.07:
                    obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'AUCTION_PRICE_RANGE_FILTER'); continue
            left_ok = g.auction_yiqian_left_ok.get(s)
            if left_ok is None:
                left_ok = _auction_yiqian_left_pressure_ok(s, context)
            if not left_ok:
                obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'AUCTION_LEFT_PRESSURE_FILTER'); continue
            score = vol_ratio * auction_ratio * (1.2 if kind == 'y2' else 1.0)
            payload = {'auction_kind': kind, 'auction_ratio': auction_ratio, 'volume_ratio': vol_ratio, 'market_cap': market_cap, 'circulating_market_cap': circ_cap, 'left_pressure_ok': left_ok, 'score': score}
            obs.mark_qualified_for_ranking(sid, payload)
            qualified.append((s, score, kind, auction_ratio))
        except Exception:
            obs.emit_block(sid, context, g, 'DATA_INVALID', 'DATA_MISSING', detail='auction exception')
            continue

    if not qualified:
        return
    qualified.sort(key=lambda x: -x[1])
    for rank, (s, score, _, _) in enumerate(qualified, start=1):
        obs.mark_ranked(_attr_sid(context, branch, s), rank=rank, score=score)
    take = min(slots, len(qualified))
    for s, _, _, _ in qualified[take:]:
        sid = _attr_sid(context, branch, s); seq = obs.emit_decision(sid, context, g, 'RANKING', 'rank_gt_take', True, False, 'RANK_OUT', 'ranked outside take', selected_for_order=False)
        obs.set_terminal_once(sid, 'RANKED_OUT', 'RANK_OUT', seq)
    value = context.portfolio.total_value * g.auction_yiqian_daily_value / take
    bought = 0
    for idx, (s, _, kind, auction_ratio) in enumerate(qualified[:take]):
        d = cd[s]
        cash = min(value, context.portfolio.available_cash)
        sid = _attr_sid(context, branch, s)
        if cash <= 5000:
            seq = obs.emit_decision(sid, context, g, 'GLOBAL_RESOURCE', 'cash_gt_5000', cash, False, 'GLOBAL_CASH_INSUFFICIENT', 'cash<=5000')
            obs.set_terminal_once(sid, 'CASH_BLOCKED', 'GLOBAL_CASH_INSUFFICIENT', seq)
            affected = [_attr_sid(context, branch, x[0]) for x in qualified[:take][idx+1:]]
            obs.emit_loop_stop(context, g, branch, handler, 'CASH_BELOW_MINIMUM', 'CASH_BELOW_MINIMUM', idx, bought, take, slots, affected)
            break
        obs.mark_selected_for_order(sid)
        o = order_value(s, cash * scale, MarketOrderStyle(d.day_open))
        if o:
            g.owner[s] = 'auction'; g.buy_mode[s] = g.market_mode
            log.info('[竞价买] %s %s auction=%.3f' % (s, kind, auction_ratio))
            bought += 1


def buy_v227_一进二(context):
    obs = _attr_observer(); branch = 'YJJ'; handler='buy_v227_一进二'; _attr_mark_handler(context, branch)
    if not g.enable_v227:
        log.info('[v227一进二] 未启用(活跃=%s)' % g.active); _attr_bulk_terminal(context, branch, 'ROUTED_OUT', 'ROUTE_DISABLED', stage='ROUTE_GATE'); return
    if _v227_shock_new_buy_blocked('v227一进二'):
        _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'SHOCK_COOLDOWN', stage='COOLDOWN_GATE'); return
    if g.market_mode == 'cautious' and 0.4 <= g.fb_pct < 0.6:
        log.info('[v227一进二] cautious+pct毒区跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'FB_PCT_BLOCK'); return
    if g.market_mode == 'bull' and g.fb_pct < 0.2:
        log.info('[v227一进二] bull+pct<0.2跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'FB_PCT_BLOCK'); return
    if g.market_mode == 'bull' and g.bull_cooldown > 0:
        log.info('[v227一进二] bull冷却期跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'BULL_COOLDOWN'); return
    if g.market_mode == 'bear':
        log.info('[v227一进二] bear模式跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'MARKET_MODE_BLOCK'); return
    if g.stoploss_cooldown > 0 and g.market_mode != 'bull':
        log.info('[v227一进二] v227冷却期跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'STOPLOSS_COOLDOWN'); return
    if not g.yjj_candidates:
        log.info('[v227一进二] 无候选'); return
    held = _held_count_by('v227', context); slots = g.v227_slots - held
    if slots <= 0:
        _attr_bulk_terminal(context, branch, 'SLOT_BLOCKED', 'BRANCH_SLOT_FULL', stage='BRANCH_SLOT'); return
    ...
    scale = _win_scale()
    if scale == 0.0:
        _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'WIN_RATE_SCALE_ZERO'); return
    pos_pct *= scale
    cd = get_current_data(); bought = 0
    for idx, item in enumerate(g.yjj_candidates):
        if bought >= slots:
            obs.emit_loop_stop(context, g, branch, handler, 'SLOTS_FILLED', 'SLOTS_FILLED', idx, bought, None, slots, _attr_remaining_sids(context, branch, g.yjj_candidates, idx)); break
        stock = item[0] if isinstance(item, tuple) else item; sid = _attr_sid(context, branch, stock); obs.mark_candidate_loop_reached(sid)
        d = cd[stock]
        if d.paused:
            obs.emit_block(sid, context, g, 'DATA_INVALID', 'PAUSED'); continue
        yc = g.yjj_yclose.get(stock, 0)
        if yc <= 0:
            obs.emit_block(sid, context, g, 'DATA_INVALID', 'DATA_MISSING', detail='yclose<=0'); continue
        if d.day_open >= d.high_limit * 0.999:
            obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'LIMIT_UP_UNBUYABLE'); continue
        open_pct = d.day_open / yc - 1
        if open_pct < -1e-6 or open_pct > open_hi:
            obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'OPEN_RANGE_FILTER', detail='open_pct=%s open_hi=%s' % (open_pct, open_hi)); continue
        if stock in context.portfolio.positions and context.portfolio.positions[stock].total_amount > 0:
            _attr_position_block(sid, context, stock); continue
        obs.mark_handler_eligible(sid, True); obs.mark_branch_eligible(sid, True); obs.mark_selected_for_order(sid)
        cash = context.portfolio.available_cash * pos_pct / max(slots - bought, 1)
        if cash > 5000:
            o = order_value(stock, cash, MarketOrderStyle(d.day_open))
            if not o:
                continue
            g.owner[stock] = 'v227'; g.buy_mode[stock] = g.market_mode
            log.info('[v227买] %s 开%.1f%% [%s]' % (stock, open_pct * 100, g.market_mode)); bought += 1
        else:
            seq = obs.emit_decision(sid, context, g, 'GLOBAL_RESOURCE', 'cash_gt_5000', cash, False, 'GLOBAL_CASH_INSUFFICIENT', 'cash<=5000')
            obs.set_terminal_once(sid, 'CASH_BLOCKED', 'GLOBAL_CASH_INSUFFICIENT', seq)
...
def buy_v227_天蝎座(context):
    obs = _attr_observer(); branch='Scorpion'; handler='buy_v227_天蝎座'; _attr_mark_handler(context, branch)
    if not g.enable_v227:
        _attr_bulk_terminal(context, branch, 'ROUTED_OUT', 'ROUTE_DISABLED', stage='ROUTE_GATE'); return
    if g.market_mode != 'bear' or not g.bear_candidates:
        _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'MARKET_MODE_BLOCK'); return
    scale = _win_scale()
    if scale == 0.0:
        _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'WIN_RATE_SCALE_ZERO'); return
    held = _held_count_by('v227', context); slots = g.v227_slots - held
    if slots <= 0:
        _attr_bulk_terminal(context, branch, 'SLOT_BLOCKED', 'BRANCH_SLOT_FULL', stage='BRANCH_SLOT'); return
    cd = get_current_data(); bought = 0
    for idx, stock in enumerate(g.bear_candidates):
        if bought >= slots:
            obs.emit_loop_stop(context, g, branch, handler, 'SLOTS_FILLED', 'SLOTS_FILLED', idx, bought, None, slots, _attr_remaining_sids(context, branch, g.bear_candidates, idx)); break
        sid = _attr_sid(context, branch, stock); obs.mark_candidate_loop_reached(sid)
        d = cd[stock]
        if d.paused:
            obs.emit_block(sid, context, g, 'DATA_INVALID', 'PAUSED'); continue
        yc = g.yjj_yclose.get(stock, 0)
        ...
        open_pct = d.day_open / yc - 1
        if open_pct < -0.04 or open_pct > -0.03:
            obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'OPEN_RANGE_FILTER', detail='open_pct=%s' % open_pct); continue
        if stock in context.portfolio.positions and context.portfolio.positions[stock].total_amount > 0:
            _attr_position_block(sid, context, stock); continue
        obs.mark_handler_eligible(sid, True); obs.mark_branch_eligible(sid, True); obs.mark_selected_for_order(sid)
        cash = context.portfolio.available_cash / max(slots - bought, 1)
        if cash > 5000:
            ...
            if not o:
                continue
            g.owner[stock] = 'v227'; g.buy_mode[stock] = g.market_mode
            log.info('[天蝎座] %s 低开%.1f%%' % (stock, open_pct * 100)); bought += 1
        else:
            seq = obs.emit_decision(sid, context, g, 'GLOBAL_RESOURCE', 'cash_gt_5000', cash, False, 'GLOBAL_CASH_INSUFFICIENT', 'cash<=5000')
            obs.set_terminal_once(sid, 'CASH_BLOCKED', 'GLOBAL_CASH_INSUFFICIENT', seq)


def _attr_buy_rzq_zb_common(context, branch, candidates, yclose_map, slot_attr, owner_label, ratio_low, ratio_high, log_prefix, scale):
    obs = _attr_observer(); cd = get_current_data(); cands = []
    for s in candidates:
        sid = _attr_sid(context, branch, s); obs.mark_candidate_loop_reached(sid)
        d = cd[s]
        if d.paused:
            obs.emit_block(sid, context, g, 'DATA_INVALID', 'PAUSED'); continue
        yc = yclose_map.get(s, 0)
        if yc <= 0:
            obs.emit_block(sid, context, g, 'DATA_INVALID', 'DATA_MISSING', detail='yclose<=0'); continue
        op = d.day_open
        if op <= 0:
        ...
        if not (ratio_low < ratio < ratio_high):
            obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'OPEN_RANGE_FILTER', detail='ratio=%s' % ratio); continue
        if d.last_price >= d.high_limit * 0.999 or d.last_price <= d.low_limit * 1.001:
            obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'LIMIT_PRICE_FILTER'); continue
        if s in context.portfolio.positions and context.portfolio.positions[s].total_amount > 0:
            _attr_position_block(sid, context, s); continue
        obs.mark_handler_eligible(sid, True); obs.mark_branch_eligible(sid, True)
        cands.append((s, op, yc))
    if not cands:
        return
    date_str = context.current_dt.strftime('%Y-%m-%d'); filtered = []
    for s, op, yc in cands:
        sid = _attr_sid(context, branch, s)
        try:
            au = get_call_auction(s, start_date=date_str, end_date=date_str)
            ...
            sell_m = sum(row.get('a%d_p' % i, 0) * row.get('a%d_v' % i, 0) for i in range(1, 6))
            if sell_m <= 0 or (buy_m - sell_m) / sell_m <= 0:
                obs.emit_block(sid, context, g, 'BRANCH_FILTERED', 'AUCTION_BUY_SELL_FILTER'); continue
        except Exception:
            obs.emit_block(sid, context, g, 'DATA_INVALID', 'DATA_MISSING', detail='auction exception'); continue
        obs.mark_qualified_for_ranking(sid, {'buy_m': buy_m, 'sell_m': sell_m})
        filtered.append((s, op, yc, buy_m, sell_m))
    if not filtered:
        return
    codes = [s for s, _, _, _, _ in filtered]
    try:
        ...
        df_val = pd.DataFrame()
    tr_map = dict(zip(df_val['code'], df_val['turnover_ratio'])) if not df_val.empty else {}
    scored = [(s, (tr_map.get(s, 0) or 0) * (op / yc) * (buy_m / sell_m if sell_m > 0 else 1.0)) for s, op, yc, buy_m, sell_m in filtered]
    scored.sort(key=lambda x: -x[1])
    ranks = {}
    for rank, (s, score) in enumerate(scored, start=1):
        ranks[s] = rank
        obs.mark_ranked(_attr_sid(context, branch, s), rank=rank, score=score)
    held = _held_count_by(owner_label, context); slots = max(0, getattr(g, slot_attr) - held)
    if slots == 0:
        for s, _ in scored:
            sid = _attr_sid(context, branch, s); seq = obs.emit_decision(sid, context, g, 'BRANCH_SLOT', 'slots_gt_0', slots, False, 'BRANCH_SLOT_FULL', 'slots==0')
            obs.set_terminal_once(sid, 'SLOT_BLOCKED', 'BRANCH_SLOT_FULL', seq)
        return
    take = min(slots, len(scored))
    bought = 0
    visited = set()
    for idx, (s, _) in enumerate(scored):
        if bought >= take:
            remaining = [_attr_sid(context, branch, x[0]) for x in scored[idx:]]
            obs.emit_loop_stop(context, g, branch, 'buy_' + owner_label, 'TAKE_FILLED', 'TAKE_FILLED', idx, bought, take, slots, remaining)
            break
        visited.add(s)
        sid = _attr_sid(context, branch, s)
        if s in context.portfolio.positions and context.portfolio.positions[s].total_amount > 0:
        ...
        if cash > 5000:
            ...
            o = order_value(s, cash * scale, MarketOrderStyle(d.day_open))
            if o:
                g.owner[s] = owner_label; g.buy_mode[s] = g.market_mode
                log.info('[%s买] %s op/yc=%.3f' % (log_prefix, s, d.day_open / yclose_map.get(s, 1)))
                bought += 1
        else:
            seq = obs.emit_decision(sid, context, g, 'GLOBAL_RESOURCE', 'cash_gt_5000', cash, False, 'GLOBAL_CASH_INSUFFICIENT', 'cash<=5000')
            obs.set_terminal_once(sid, 'CASH_BLOCKED', 'GLOBAL_CASH_INSUFFICIENT', seq)
    for s, _ in scored:
        sid = _attr_sid(context, branch, s)
        row = getattr(obs, 'signals_by_id', {}).get(sid, {})
        if row.get('terminal_state') != 'UNRESOLVED':
            continue
        if ranks.get(s, 0) > take and s not in visited:
            seq = obs.emit_decision(sid, context, g, 'RANKING', 'rank_gt_take', ranks.get(s), False, 'RANK_OUT', 'ranked outside take')
            obs.set_terminal_once(sid, 'RANKED_OUT', 'RANK_OUT', seq)
        elif s not in visited:
            seq = obs.emit_decision(sid, context, g, 'LOOP_CONTROL', 'loop_reached', False, False, 'NOT_EVALUATED_AFTER_STOP', 'loop stopped before candidate')
            obs.set_terminal_once(sid, 'NOT_EVALUATED_AFTER_STOP', 'NOT_EVALUATED_AFTER_STOP', seq)


def buy_rzq(context):
    branch='RZQ'; _attr_mark_handler(context, branch)
    if not g.enable_rzq:
        log.info('[rzq] 未启用(活跃=%s)' % g.active); _attr_bulk_terminal(context, branch, 'ROUTED_OUT', 'ROUTE_DISABLED', stage='ROUTE_GATE'); return
    if g.market_mode == 'cautious' and 0.4 <= g.fb_pct < 0.6:
        log.info('[rzq] cautious+pct毒区跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'FB_PCT_BLOCK'); return
    if g.market_mode == 'bull' and 0.4 <= g.fb_pct < 0.6:
        log.info('[rzq] bull+pct毒区跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'FB_PCT_BLOCK'); return
    if g.market_mode == 'bull' and g.bull_cooldown > 0:
        log.info('[rzq] bull冷却期跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'BULL_COOLDOWN'); return
    if _is_pass_month(context):
        log.info('[rzq] 空仓月跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'MONTH_BLOCK'); return
    if g.rzq_cooldown > 0:
        log.info('[rzq] rzq冷却期跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'RZQ_COOLDOWN'); return
    if not g.rzq_candidates:
        log.info('[rzq] 无候选'); return
    scale = _win_scale()
    if scale == 0.0:
        _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'WIN_RATE_SCALE_ZERO'); return
    _attr_buy_rzq_zb_common(context, 'RZQ', g.rzq_candidates, g.rzq_yclose, 'rzq_slots', 'rzq', 0.96, 1.01, 'rzq', scale)


def buy_zb(context):
    branch='ZB'; _attr_mark_handler(context, branch)
    if not g.enable_zb:
        log.info('[zb] 未启用(活跃=%s)' % g.active); _attr_bulk_terminal(context, branch, 'ROUTED_OUT', 'ROUTE_DISABLED', stage='ROUTE_GATE'); return
    if g.market_mode == 'cautious' and 0.4 <= g.fb_pct < 0.6:
        log.info('[zb] cautious+pct毒区跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'FB_PCT_BLOCK'); return
    if g.market_mode == 'bull' and g.bull_cooldown > 0:
        log.info('[zb] bull冷却期跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'BULL_COOLDOWN'); return
    if _is_pass_month(context):
        log.info('[zb] 空仓月跳过'); _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'MONTH_BLOCK'); return
    if not g.zb_candidates:
        log.info('[zb] 无候选'); return
    scale = _win_scale()
    if scale == 0.0:
        _attr_bulk_terminal(context, branch, 'MOTHERBOARD_GATED_OUT', 'WIN_RATE_SCALE_ZERO'); return
    _attr_buy_rzq_zb_common(context, 'ZB', g.zb_candidates, g.zb_yclose, 'zb_slots', 'zb', 0.97, 1.075, 'zb', scale)
# === PHASE1B_ATTRIBUTION_BUY_OVERRIDES_END ===



# === PHASE1C_SCAN_SOURCE_OVERRIDES_BEGIN ===
_phase1c_original_prepare_all = prepare_all
_phase1c_original_scan_all = _scan_all
_phase1c_original_scan_boards_for_prev = _scan_boards_for_prev
_phase1c_original_rzq_prepare = _rzq_prepare
_phase1c_original_zb_prepare = _zb_prepare
_phase1c_original_auction_yiqian_prepare = _auction_yiqian_prepare


def _phase1c_obs():
    return _attr_observer()


def _phase1c_emit_prepared_parent(context, branch, code, pattern_variant, source_mode, payload=None):
    obs = _phase1c_obs()
    sid = obs.prepared_signal_id_for(context, branch, code)
    pid = obs.emit_raw_pattern(
        context, g, branch, code, pattern_variant,
        record_type='OBSERVED_RAW_PATTERN',
        pattern_detected=True,
        source_mode=source_mode,
        source_coverage='COMPLETE_ACTUAL_PATH',
        payload=payload or {},
        prepared_signal_id=sid,
        survived_to_prepared=True,
        scan_terminal_state='PREPARED',
        scan_terminal_reason='entered prepared candidate list',
    )
    obs.emit_scan_decision(pid, 'PREPARED_OUTPUT', 'entered_prepared_candidates', True, True, source_function='phase1c_post_scan_parent')
    obs.align_pattern_prepared(context, branch, code, pattern_variant, sid, 'ONE_TO_ONE', 'prepared candidate has observed parent')


def _phase1c_emit_source_limited_prepared(context, code):
    obs = _phase1c_obs()
    kind = None
    try:
    ...
    declared = None
    if kind == 'y2':
        declared = 'AUCTION_Y2_BASE_PATTERN'
    elif kind == 'rzq':
        declared = 'AUCTION_RZQ_BASE_PATTERN'
    sid = obs.prepared_signal_id_for(context, 'Auction', code)
    payload = {
        'kind': kind,
        'prev_close': getattr(g, 'auction_yiqian_yclose', {}).get(code),
    ...
    variant = 'AUCTION_CACHE_PREPARED_SOURCE'
    pid = obs.emit_raw_pattern(
        context, g, 'Auction', code, variant,
        record_type='SOURCE_LIMITED_PREPARED_RECORD',
        pattern_detected=None,
        source_mode='PROJECT_AUCTION_PREPARE_CACHE',
        source_coverage='PREPARED_ONLY',
        payload=payload,
        prepared_signal_id=sid,
        survived_to_prepared=True,
        scan_terminal_state='SOURCE_LIMITED',
        scan_terminal_reason='cache exposes prepared candidate only',
        declared_pattern_variant=declared,
    )
    obs.emit_scan_decision(pid, 'PREPARED_OUTPUT', 'cache_prepared_record', True, True, reason_code='SOURCE_LIMITED', source_function='phase1c_auction_cache_parent')
    obs.align_pattern_prepared(context, 'Auction', code, variant, sid, 'SOURCE_LIMITED', 'prepared candidate mapped to source-limited cache record')


def prepare_all(context):
    obs = _phase1c_obs()
    obs.start_scan_day(context, g)
    try:
        return _phase1c_original_prepare_all(context)
    finally:
        obs.finalize_scan_day(context, g)


def _scan_all(context):
    obs = _phase1c_obs()
    obs.update_scan_run(context, g, 'YJJ', scanner_function='_scan_all', scanner_invoked=True,
                        scan_status='EXECUTED_EMPTY', control_flow_reason='market_mode is not bear',
                        source_mode='PROJECT_BOARD_SNAPSHOT', source_coverage='COMPLETE_ACTUAL_PATH')
    result = _phase1c_original_scan_all(context)
    cands = [x[0] if isinstance(x, tuple) else x for x in (getattr(g, 'yjj_candidates', []) or [])]
    for code in cands:
        _phase1c_emit_prepared_parent(context, 'YJJ', code, 'YJJ_FIRST_BOARD', 'PROJECT_BOARD_SNAPSHOT', {'prepared_rank_source': 'post_scan_candidate_list'})
    obs.update_scan_run(context, g, 'YJJ', scan_status='EXECUTED' if cands else 'EXECUTED_EMPTY',
                        raw_pattern_count=len(cands), prepared_candidate_count=len(cands))
    return result


def _scan_boards_for_prev(context):
    obs = _phase1c_obs()
    is_bear = getattr(g, 'market_mode', None) == 'bear'
    obs.update_scan_run(context, g, 'Scorpion', scanner_function='_scan_boards_for_prev', scanner_invoked=True,
                        scan_status='EXECUTED_EMPTY' if is_bear else 'EARLY_RETURN_NO_SOURCE',
                        control_flow_reason='bear mode first-board scan' if is_bear else 'prev-board scan called for v227 state, scorpion generation not active',
                        source_mode='PROJECT_BOARD_SNAPSHOT', source_coverage='COMPLETE_ACTUAL_PATH')
    result = _phase1c_original_scan_boards_for_prev(context)
    cands = [x[0] if isinstance(x, tuple) else x for x in (getattr(g, 'bear_candidates', []) or [])]
    for code in cands:
        _phase1c_emit_prepared_parent(context, 'Scorpion', code, 'SCORPION_FIRST_BOARD', 'PROJECT_BOARD_SNAPSHOT', {'prepared_rank_source': 'post_scan_candidate_list'})
    obs.update_scan_run(context, g, 'Scorpion', scan_status='EXECUTED' if cands else 'EXECUTED_EMPTY',
                        raw_pattern_count=len(cands), prepared_candidate_count=len(cands))
    return result
...
def _rzq_prepare(context):
    obs = _phase1c_obs()
    obs.update_scan_run(context, g, 'RZQ', scanner_function='_rzq_prepare', scanner_invoked=True,
                        scan_status='EXECUTED_EMPTY', control_flow_reason='not pass month',
                        source_mode='BILLBOARD_API', source_coverage='COMPLETE_ACTUAL_PATH')
    result = _phase1c_original_rzq_prepare(context)
    cands = list(getattr(g, 'rzq_candidates', []) or [])
    for code in cands:
        _phase1c_emit_prepared_parent(context, 'RZQ', code, 'RZQ_BILLBOARD_BROKEN_BOARD', 'BILLBOARD_API', {'prepared_rank_source': 'post_scan_candidate_list'})
    obs.update_scan_run(context, g, 'RZQ', scan_status='EXECUTED' if cands else 'EXECUTED_EMPTY',
                        raw_pattern_count=len(cands), prepared_candidate_count=len(cands))
    return result
...
def _zb_prepare(context):
    obs = _phase1c_obs()
    obs.update_scan_run(context, g, 'ZB', scanner_function='_zb_prepare', scanner_invoked=True,
                        scan_status='EXECUTED_EMPTY', control_flow_reason='not pass month and bull market_mode',
                        source_mode='DAILY_PRICE_COMPUTED', source_coverage='COMPLETE_ACTUAL_PATH')
    result = _phase1c_original_zb_prepare(context)
    cands = list(getattr(g, 'zb_candidates', []) or [])
    for code in cands:
        _phase1c_emit_prepared_parent(context, 'ZB', code, 'ZB_BROKEN_BOARD', 'DAILY_PRICE_COMPUTED', {'prepared_rank_source': 'post_scan_candidate_list'})
    obs.update_scan_run(context, g, 'ZB', scan_status='EXECUTED' if cands else 'EXECUTED_EMPTY',
                        raw_pattern_count=len(cands), prepared_candidate_count=len(cands))
    return result
...
def _auction_yiqian_prepare(context):
    ...
    obs.update_scan_run(context, g, 'Auction', scanner_function='_auction_yiqian_prepare', scanner_invoked=True,
                        scan_status='SOURCE_LIMITED', control_flow_reason='auction sleeve enabled',
                        source_mode='PROJECT_AUCTION_PREPARE_CACHE', source_coverage='PREPARED_ONLY', raw_pattern_count=0)
    result = _phase1c_original_auction_yiqian_prepare(context)
    cands = list(getattr(g, 'auction_yiqian_candidates', []) or [])
    for code in cands:
        _phase1c_emit_source_limited_prepared(context, code)
    obs.update_scan_run(context, g, 'Auction', scan_status='SOURCE_LIMITED' if cands else 'EXECUTED_EMPTY',
                        source_limited_prepared_record_count=len(cands), prepared_candidate_count=len(cands), raw_pattern_count=0)
    return result
# === PHASE1C_SCAN_SOURCE_OVERRIDES_END ===
