...
INSTRUMENTED_STRATEGY = ROOT / "research" / "instrumented_strategies" / "motherboard_phase1b_observed.py"
DEFAULT_OUT = ROOT / "coordination" / "attribution" / "master_phase1b"
PHASE1A_OUT_CANDIDATES = [
    ROOT.parent / "motherboard-attribution-phase1a-v1" / "coordination" / "attribution" / "master_phase1a",
    ROOT / "coordination" / "attribution" / "master_phase1a",
]
...
def phase1a_alignment(out_dir: Path, phase1b_cases: dict, signal_events: list[dict]) -> dict:
    phase1a_dir = next((p for p in PHASE1A_OUT_CANDIDATES if p.exists()), None)
    result = {"phase1a_dir": str(phase1a_dir) if phase1a_dir else None, "available": phase1a_dir is not None}
    if phase1a_dir is None:
        return result
    result["trades_equal"] = frames_equal(read_table(phase1a_dir / "B0_formal" / "trades.csv"), phase1b_cases["B0_formal"]["trades"])
    result["orders_equal"] = frames_equal(read_table(phase1a_dir / "B0_formal" / "orders.csv"), phase1b_cases["B0_formal"]["orders"])
    p1a_signal = read_table(phase1a_dir / "SIGNAL_EVENT.parquet")
    if p1a_signal.empty:
        p1a_signal = read_table(phase1a_dir / "SIGNAL_EVENT_SAMPLE.csv")
    ...
    if all(c in p1a_signal.columns for c in key_cols) and all(c in p1b_signal.columns for c in key_cols):
        ...
        result.update({
            ...
            "phase1a_signal_keys": len(k1),
            "phase1b_signal_keys": len(k2),
            "phase1a_minus_phase1b_sample": ["|".join(x) for x in sorted(k1 - k2)[:20]],
            "phase1b_minus_phase1a_sample": ["|".join(x) for x in sorted(k2 - k1)[:20]],
        })
    else:
        result["signal_keys_equal"] = None
        result["signal_key_note"] = "Phase 1A full SIGNAL_EVENT not found or lacks v0.3 key columns."
    return result
...
def write_report(path: Path, status: str, args, schema_version: str, parity: dict, mapping: dict, closure: dict, alignment: dict, inst_diff: dict):
    report = f"""# Phase 1B Report
...
def main():
    ...
    alignment = phase1a_alignment(out_dir, cases, obs.signal_events)
    ...
    write_json(alignment, out_dir / "PHASE1A_PHASE1B_ALIGNMENT.json")
    ...
    manifest = {
        ...
        "status": status,
        "artifacts": artifacts,
        "command": "python research/run_motherboard_phase1b.py --start %s --end %s" % (args.start, args.end),
    }
    write_json(manifest, out_dir / "RUN_MANIFEST.json")
    write_report(out_dir / "PHASE1B_REPORT.md", status, args, schema_version, parity, mapping, closure, alignment, inst_diff)

    print(json.dumps({"status": status, "closure": closure, "mapping": mapping, "parity": parity, "alignment": alignment}, ensure_ascii=False, indent=2))
