...
def _audit_v227(context, event, stock, price, reason='', extra=''):
    try:
        log.info('AUDIT_V227_%s,%s,%s,%.3f,%s,%s,%.6f,%.6f,%s' % (
            event,
            context.current_dt.strftime('%Y%m%d %H:%M:%S'),
            stock,
            float(price) if price is not None else 0.0,
            reason,
            getattr(g, 'market_mode', ''),
            float(getattr(g, 'fb_pct', 0.0)),
            float(getattr(g, 'first_board_perf', 0.0)),
            extra,
        ))
    except Exception as e:
        log.info('AUDIT_V227_ERROR,%s,%s,%s' % (event, stock, e))



def _audit_v227_check(context, stage, stock, pos, d, note=''):
    try:
        avg = float(getattr(pos, 'avg_cost', 0) or 0)
        last = float(getattr(d, 'last_price', 0) or 0)
        hl = float(getattr(d, 'high_limit', 0) or 0)
        ll = float(getattr(d, 'low_limit', 0) or 0)
        ret = last / avg - 1 if avg > 0 else 0.0
        log.info('AUDIT_V227_CHECK,%s,%s,%s,%.3f,%.3f,%.6f,%.3f,%.3f,%s,%s,%.6f,%.6f' % (
            stage, context.current_dt.strftime('%Y%m%d %H:%M:%S'), stock,
            last, avg, ret, hl, ll, note, getattr(g, 'market_mode', ''),
            float(getattr(g, 'fb_pct', 0.0)), float(getattr(g, 'first_board_perf', 0.0))))
    except Exception as e:
        log.info('AUDIT_V227_CHECK_ERROR,%s,%s,%s' % (stage, stock, e))
...
def buy_v227_一进二(context):
    ...
    for item in g.yjj_candidates:
        ...
        if cash > 5000:
            ...
            _audit_v227(context, 'BUY', stock, d.day_open, 'v227_yjj', 'open_pct=%.6f;slots=%d;held=%d' % (open_pct, slots, held))
            bought += 1
...
def sell_v227_morning(context):
    ...
    for s in list(context.portfolio.positions):
        ...
        if pos.closeable_amount <= 0:
            continue
        d = cd[s]
        _audit_v227_check(context, 'morning', s, pos, d, 'precheck')
        if d.last_price >= d.high_limit * 0.999:
            continue
        ...
        if pos.avg_cost > 0 and d.last_price > pos.avg_cost:
            ...
            _audit_v227(context, 'SELL', s, d.last_price, 'morning_profit', 'avg_cost=%.6f;ret=%.6f' % (pos.avg_cost, d.last_price / pos.avg_cost - 1 if pos.avg_cost > 0 else 0))
    ...
    for s in list(context.portfolio.positions):
        ...
        if owner == 'v227':
            ...
            if s in g.dieting:
                ...
                _audit_v227(context, 'SELL', s, d.last_price, 'limit_down_reopen', 'avg_cost=%.6f;ret=%.6f' % (pos.avg_cost, ret))
                continue
            if d.last_price <= d.low_limit * 1.001:
                g.dieting.append(s)
            ...
            if ret <= g.v227_stop:
                ...
                _audit_v227(context, 'SELL', s, d.last_price, 'stop_loss', 'avg_cost=%.6f;ret=%.6f' % (pos.avg_cost, ret))
...
def sell_v227_midday(context):
    ...
    for s in list(context.portfolio.positions):
        ...
        if pos.closeable_amount <= 0 or pos.avg_cost <= 0:
            continue
        d = cd[s]
        _audit_v227_check(context, 'midday', s, pos, d, 'precheck')
        if d.last_price >= d.high_limit * 0.999:
            continue
        if d.last_price <= d.low_limit * 1.001:
            if s not in g.dieting:
        ...
        if ret <= -0.02:
            ...
            _audit_v227(context, 'SELL', s, d.last_price, 'midday_loss', 'avg_cost=%.6f;ret=%.6f' % (pos.avg_cost, ret))
...
def sell_v227_afternoon(context):
    ...
    for s in list(context.portfolio.positions):
        ...
        if pos.closeable_amount <= 0:
            continue
        d = cd[s]
        _audit_v227_check(context, 'afternoon', s, pos, d, 'precheck')
        if d.last_price >= d.high_limit * 0.999:
            continue
        if d.last_price <= d.low_limit * 1.001:
            if s not in g.dieting:
        ...
        if pos.avg_cost > 0:
            ...
            _audit_v227(context, 'SELL', s, d.last_price, 'eod_clear', 'avg_cost=%.6f;ret=%.6f' % (pos.avg_cost, d.last_price / pos.avg_cost - 1 if pos.avg_cost > 0 else 0))
