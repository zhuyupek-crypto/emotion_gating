...
def collect_call_sites(patterns: list[str]) -> tuple[list[str], list[str]]:
    ...
    return runtime_hits, secondary_hits


def build_trigger_info(data_obj: Any = None, manual_dates: list[str] | None = None, manual_codes: list[str] | None = None) -> tuple[list[str], list[str]]:
...
HOOK_SPECS: list[HookSpec] = [
    ...
    HookSpec(
        ...
        status="investigation_pending",
        call_site_patterns=["from rebuild_from_archive.data_api", "import rebuild_from_archive.data_api", "rebuild_from_archive/data_api.py"],
        count_override=1,
    ),
    HookSpec(
...
def filter_hooks(inventory: list[dict[str, Any]], *, semantic_type: str | None = None, disposition: str | None = None, status: str | None = None) -> list[dict[str, Any]]:
    rows = inventory
...
def build_answers(inventory: list[dict[str, Any]]) -> dict[str, list[str]]:
    ...
    only_nav = [
        ...
        for row in inventory
        if row["affects_nav"]
        and not row["affects_selection"]
        and not row["affects_state"]
        and not row["hook_id"].startswith("execution.preopen_")
        and row["hook_id"] != "engine.immediate_sell_cash_release"
    ]
    affects_branch = [
        row["hook_id"]
        for row in inventory
        if row["affects_selection"] or row["affects_state"] or row["affects_order"]
    ]
    local_native_disable_first = [
        row["hook_id"]
        for row in inventory
        if row["disposition"] == "archive_jq_only"
        and not row["affects_selection"]
        and not row["affects_state"]
        and row["runtime_callsite_count"] > 0
        and not row["hook_id"].startswith("execution.preopen_")
    ]
    return {
        "general_market_rules": dedupe(ids(filter_hooks(inventory, semantic_type="market_rule"))),
        "suspected_hdata_errors": dedupe(ids([row for row in inventory if row["disposition"] == "move_to_hdata"])),
        ...
        "still_unknown": dedupe(ids(filter_hooks(inventory, semantic_type="unknown")) + ids(filter_hooks(inventory, disposition="investigate"))),
        "disable_only_price_size_nav": dedupe(only_nav),
        "disable_affects_candidates_state_branches": dedupe(affects_branch),
        "local_native_disable_first": dedupe(local_native_disable_first),
        "must_wait_for_local_quant": dedupe(ids([row for row in inventory if row["disposition"] == "move_to_local_quant"])),
        "must_wait_for_hdata": dedupe(ids([row for row in inventory if row["disposition"] == "move_to_hdata"])),
    }
...
def render_inventory_markdown(inventory: list[dict[str, Any]], summary: dict[str, Any], answers: dict[str, list[str]]) -> str:
    ...
    qa_map = [
        ...
        ("2. 哪些钩子疑似HData错误？", "suspected_hdata_errors"),
        ("3. 哪些钩子只是聚宽历史复刻？", "jq_history_replay_only"),
        ("4. 哪些钩子真正属于项目逻辑？", "project_logic"),
        ("5. 哪些钩子仍无法判断？", "still_unknown"),
        ("6. 哪些钩子关闭后只影响价格、股数和净值？", "disable_only_price_size_nav"),
        ("7. 哪些钩子关闭后会影响候选、状态和分支触发？", "disable_affects_candidates_state_branches"),
        ("8. 建立local-native模式时，建议先关闭哪一组？", "local_native_disable_first"),
        ("9. 哪些钩子必须等待local_quant能力完成后才能删除？", "must_wait_for_local_quant"),
        ("10. 哪些钩子必须等待HData发布修复版本后才能删除？", "must_wait_for_hdata"),
    ]
    ...
    lines.append("")
    lines.append("| hook_id | semantic_type | disposition | status | years | codes | runtime call sites | target_owner |")
    lines.append("| --- | --- | --- | --- | --- | --- | ---: | --- |")
    for row in inventory:
        lines.append(
            f"| `{row['hook_id']}` | `{row['semantic_type']}` | `{row['disposition']}` | `{row['status']}` | "
            f"`{','.join(str(y) for y in row['year_tags'])}` | `{','.join(row['trigger_codes'][:6])}` | "
            f"`{row['runtime_callsite_count']}` | `{row['target_owner']}` |"
        )
    lines.append("")
...
def render_archive_plan_markdown(inventory: list[dict[str, Any]]) -> str:
    rows = [row for row in inventory if row["disposition"] == "archive_jq_only"]
    lines = [
        "# JQ Archive Plan",
        "",
    ...
    first_wave = [
        row for row in rows
        if not row["affects_selection"] and not row["affects_state"]
    ]
    if first_wave:
        for row in first_wave:
            lines.append(f"- `{row['hook_id']}`: disable first when local-native mode is allowed to drift only in price/size/fill/NAV.")
    else:
        lines.append("- none")
    lines.append("")
    lines.append("## Remaining archive-only hooks")
    lines.append("")
    for row in rows:
        lines.append(f"## `{row['hook_id']}`")
        lines.append(f"- reason: {row['reason']}")
...
def write_csv(path: Path, inventory: list[dict[str, Any]]) -> None:
    columns = [
        ...
        "entry_count",
        "year_tags",
        "runtime_callsite_count",
        "secondary_reference_count",
        "unused_runtime",
    ]
...
def write_outputs(out_dir: Path, inventory: list[dict[str, Any]], summary: dict[str, Any], answers: dict[str, list[str]]) -> dict[str, str]:
    ...
    md_path = out_dir / "HHOOK_INVENTORY.md"
    ...
    local_quant_rows = [row for row in inventory if row["disposition"] == "move_to_local_quant"]
    hdata_rows = [row for row in inventory if row["disposition"] == "move_to_hdata"]
    local_quant_path.write_text(
