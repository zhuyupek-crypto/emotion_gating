...
from jqdata import *
# DIAG loud mode: no log.set_level monkey patch.
...
def initialize(context):
    ...
    # Force one branch for independent backtests:
    # normal / v227 / rzq / zb / rzq_zb / auction
    g.branch_test = 'v227'
...
def prepare_all(context):
    ...
    if getattr(g, 'branch_test', 'normal') in ('zb', 'rzq_zb') and not _is_pass_month(context) and not g.zb_candidates:
        _zb_prepare(context)
    ...
    if getattr(g, 'branch_test', 'normal') == 'auction':
...
def _apply_branch_test_override(context):
    ...
    if bt == 'v227':
        ...
        g.active = 'rzq_zb_only'
        g.enable_rzq = True
        g.enable_zb = True
        g.rzq_slots = 3
        g.zb_slots = 3
    elif bt == 'auction':
        g.active = 'auction_only'
    else:
...
def initialize(context):
    ...
    raise Exception('DIAG_FORCE_LOADED')
