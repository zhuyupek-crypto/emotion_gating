...
def initialize(context):
    ...
    # Branch attribution backtest modes:
    # normal: original mixed strategy.
    # force_v227 / force_rzq / force_zb / force_rzq_zb / force_auction:
    #   ignore the master route and force only this branch to open new positions.
    # mix_v227 / mix_rzq / mix_zb / mix_rzq_zb / mix_auction:
    #   keep the master route; trade this branch only on days the mixed strategy would enable it.
    # Backward-compatible aliases: v227/rzq/zb/rzq_zb/auction mean force_*.
    g.branch_test = 'force_v227'
...
def prepare_all(context):
    ...
    # In forced zb tests, build zb candidates outside bull as well.
    if getattr(g, 'branch_test', 'normal') in ('zb', 'rzq_zb', 'force_zb', 'force_rzq_zb') and not _is_pass_month(context) and not g.zb_candidates:
        _zb_prepare(context)
    ...
    _apply_branch_test_override(context)

    # v215: bull 释放确认只限制 rzq/zb 核心腿；竞价袖套是独立短线补位，仍按 v204 动态仓位运行。
    if getattr(g, 'branch_test', 'normal') in ('auction', 'force_auction'):
        g.auction_yiqian_daily_value = 1.0
    elif getattr(g, 'branch_test', 'normal') == 'mix_auction':
        g.auction_yiqian_daily_value = _auction_yiqian_dynamic_value(context)
    elif getattr(g, 'branch_test', 'normal') != 'normal':
        g.auction_yiqian_daily_value = 0.0
    else:
        g.auction_yiqian_daily_value = _auction_yiqian_dynamic_value(context)
...
def _apply_branch_test_override(context):
    """Override opening permissions for branch attribution backtests.

    Set g.branch_test in initialize:
    normal / force_* / mix_*.
    """
    bt = getattr(g, 'branch_test', 'normal')
    if bt == 'normal':
        return

    route_active = getattr(g, 'route_active', g.active)
    ...
    if bt in ('v227', 'force_v227'):
        g.active = 'force_v227'
        g.enable_v227 = True
        g.v227_slots = 2
    elif bt in ('rzq', 'force_rzq'):
        g.active = 'force_rzq'
        g.enable_rzq = True
        g.rzq_slots = 3
    elif bt in ('zb', 'force_zb'):
        g.active = 'force_zb'
        g.enable_zb = True
        g.zb_slots = 3
    elif bt in ('rzq_zb', 'force_rzq_zb'):
        g.active = 'force_rzq_zb'
        g.enable_rzq = True
        g.enable_zb = True
        g.rzq_slots = 3
        g.zb_slots = 3
    elif bt in ('auction', 'force_auction'):
        g.active = 'force_auction'
    elif bt == 'mix_v227':
        g.active = 'mix_v227' if route_active == 'v227' else 'mix_v227_idle'
        if route_active == 'v227':
            g.enable_v227 = True
            g.v227_slots = 2
    elif bt == 'mix_rzq':
        g.active = 'mix_rzq' if route_active == 'rzq+zb' else 'mix_rzq_idle'
        if route_active == 'rzq+zb':
            g.enable_rzq = True
            g.rzq_slots = 3
    elif bt == 'mix_zb':
        g.active = 'mix_zb' if route_active == 'rzq+zb' else 'mix_zb_idle'
        if route_active == 'rzq+zb':
            g.enable_zb = True
            g.zb_slots = 3
    elif bt == 'mix_rzq_zb':
        g.active = 'mix_rzq_zb' if route_active == 'rzq+zb' else 'mix_rzq_zb_idle'
        if route_active == 'rzq+zb':
            g.enable_rzq = True
            g.enable_zb = True
            g.rzq_slots = 3
        ...
        g.active = 'mix_auction'
    else:
        log.warning('[branch_test] unknown mode: %s, fallback to normal route' % bt)
        g.active = route_active
        g.enable_v227 = (route_active == 'v227')
        g.enable_rzq = (route_active == 'rzq+zb')
        g.enable_zb = (route_active == 'rzq+zb')
        if g.enable_v227:
            g.v227_slots, g.rzq_slots, g.zb_slots = 2, 0, 0
        elif g.enable_rzq or g.enable_zb:
            g.v227_slots, g.rzq_slots, g.zb_slots = 0, 3, 3
...
def _scan_all(context):
    ...
    first_boards = []
    max_b = 0
    trace_base_day = context.current_dt.strftime('%Y-%m-%d') == '2020-07-14'
    trace_base_watch = set(['600711.XSHG', '300088.XSHE', '002385.XSHE'])
    for s in all_stocks:
        trace_this = trace_base_day and s in trace_base_watch
        hl = high_limits.get(s)
        cr = closes_raw.get(s)
        if hl is None or cr is None or len(hl) < 3 or len(cr) < 3:
            if trace_this:
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s drop=no_hl_or_close hl_len=%s cr_len=%s' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s,
                    'NA' if hl is None else len(hl),
                    'NA' if cr is None else len(cr)))
            continue
        if hl[-1] <= 0 or abs(cr[-1] - hl[-1]) > 0.02:
            if trace_this:
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s drop=not_limit close=%.4f high_limit=%.4f diff=%.6f' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s,
                    cr[-1], hl[-1], abs(cr[-1] - hl[-1])))
            continue
        boards = 1
        ...
        if trace_this:
            log.info('[SM-YJJ-BASE-TRACE] %s stock=%s pass=limit boards=%d is_first=%s close=%.4f high_limit=%.4f prev_close=%.4f prev_hl=%.4f' % (
                context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, boards, is_first,
                cr[-1], hl[-1], cr[-2], hl[-2]))
        ...
        if s in secs.index:
            ...
            if 'ST' in name or 'st' in name:
                if trace_this:
                    log.info('[SM-YJJ-BASE-TRACE] %s stock=%s drop=st_name name=%s' % (
                        context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, name))
                continue
            if (context.current_dt.date() - secs.loc[s, 'start_date']).days < g.ipo_days:
                if trace_this:
                    age_days = (context.current_dt.date() - secs.loc[s, 'start_date']).days
                    log.info('[SM-YJJ-BASE-TRACE] %s stock=%s drop=ipo_age name=%s start=%s age=%d need=%d' % (
                        context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, name,
                        secs.loc[s, 'start_date'], age_days, g.ipo_days))
                continue
            if trace_this:
                age_days = (context.current_dt.date() - secs.loc[s, 'start_date']).days
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s pass=name_age name=%s start=%s age=%d' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, name,
                    secs.loc[s, 'start_date'], age_days))
        elif trace_this:
            log.info('[SM-YJJ-BASE-TRACE] %s stock=%s warn=not_in_secs' % (
                context.current_dt.strftime('%Y%m%d %H:%M:%S'), s))
        if s not in valid_caps:
            if trace_this:
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s drop=cap_not_valid valid_caps_n=%d' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, len(valid_caps)))
            continue
        if trace_this:
            cap_val = None
            try:
                cap_row = val_df[val_df['code'] == s]
                if not cap_row.empty:
                    cap_val = cap_row.iloc[0]['circulating_market_cap']
            except Exception:
                cap_val = None
            log.info('[SM-YJJ-BASE-TRACE] %s stock=%s pass=cap cap=%s' % (
                context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, str(cap_val)))
        m = moneys.get(s)
        if m is None or len(m) == 0 or m[-1] < 6e8:
            if trace_this:
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s drop=money money=%s' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s,
                    'NA' if m is None or len(m) == 0 else str(m[-1])))
            continue
        if g.market_mode == 'bull' and m[-1] > 20e8:
            if trace_this:
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s drop=bull_money money=%.2f mode=%s' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, m[-1], g.market_mode))
            continue
        if trace_this:
            v_tmp = volumes.get(s)
            v_tmp_last = 'NA' if v_tmp is None or len(v_tmp) == 0 else str(v_tmp[-1])
            avg_tmp = 'NA'
            if v_tmp is not None and len(v_tmp) > 0 and v_tmp[-1] > 0 and cr[-1] > 0:
                avg_tmp = '%.9f' % (m[-1] / v_tmp[-1] / cr[-1] * 1.1 - 1)
            log.info('[SM-YJJ-BASE-TRACE-V3] %s stock=%s pass=money money=%.2f mode=%s volume_last=%s close=%.4f avg_dbg=%s is_first=%s' % (
                context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, m[-1], g.market_mode,
                v_tmp_last, cr[-1], avg_tmp, is_first))
        ...
        if trace_this:
            v_dbg = volumes.get(s)
            try:
                v_len_dbg = 'NA' if v_dbg is None else len(v_dbg)
                v_last_dbg = 'NA' if v_dbg is None or len(v_dbg) == 0 else str(v_dbg[-1])
                avg_dbg = 'NA'
                if v_dbg is not None and len(v_dbg) > 0 and v_dbg[-1] > 0 and cr[-1] > 0:
                    avg_dbg = '%.9f' % (m[-1] / v_dbg[-1] / cr[-1] * 1.1 - 1)
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s pre_avg_check is_first=%s volume_len=%s volume_last=%s money=%.2f close=%.4f avg_dbg=%s' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, is_first,
                    v_len_dbg, v_last_dbg, m[-1], cr[-1], avg_dbg))
            except Exception as e:
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s pre_avg_check_error err=%s' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s, str(e)))
        if is_first:
            v_arr = volumes.get(s)
            if v_arr is not None and len(v_arr) > 0 and v_arr[-1] > 0 and cr[-1] > 0:
                ...
                if avg_chg < 0.07:
                    if trace_this:
                        log.info('[SM-YJJ-BASE-TRACE] %s stock=%s drop=avg_chg avg_chg=%.6f money=%.2f volume=%.2f close=%.4f' % (
                            context.current_dt.strftime('%Y%m%d %H:%M:%S'), s,
                            avg_chg, m[-1], v_arr[-1], cr[-1]))
                    continue
                if trace_this:
                    log.info('[SM-YJJ-BASE-TRACE] %s stock=%s pass=avg_chg avg_chg=%.6f money=%.2f volume=%.2f close=%.4f' % (
                        context.current_dt.strftime('%Y%m%d %H:%M:%S'), s,
                        avg_chg, m[-1], v_arr[-1], cr[-1]))
            elif trace_this:
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s warn=avg_chg_skipped volume=%s close=%.4f' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s,
                    'NA' if v_arr is None or len(v_arr) == 0 else str(v_arr[-1]), cr[-1]))
        g.yjj_yclose[s] = cr[-1]
        if is_first:
            g.yjj_candidates.append(s)
            if trace_this:
                log.info('[SM-YJJ-BASE-TRACE] %s stock=%s pass=base_append' % (
                    context.current_dt.strftime('%Y%m%d %H:%M:%S'), s))

    g.prev_first_boards = first_boards
    g._today_max_boards = max_b
    if context.current_dt.strftime('%Y-%m-%d') == '2020-07-14':
        log.info('[SM-YJJ-STAGE] %s base n=%d has600711=%s list=%s' % (
            context.current_dt.strftime('%Y%m%d %H:%M:%S'),
            len(g.yjj_candidates),
            '600711.XSHG' in [_sm_code(x) for x in g.yjj_candidates],
            _sm_codes(g.yjj_candidates),
        ))
    ...
    if g.yjj_candidates:
        ...
        if context.current_dt.strftime('%Y-%m-%d') == '2020-07-14':
            log.info('[SM-YJJ-STAGE] %s after_v122 n=%d removed=%d has600711=%s list=%s' % (
                context.current_dt.strftime('%Y%m%d %H:%M:%S'),
                len(g.yjj_candidates),
                removed,
                '600711.XSHG' in [_sm_code(x) for x in g.yjj_candidates],
    ...
    if g.yjj_candidates:
        ...
        if context.current_dt.strftime('%Y-%m-%d') == '2020-07-14':
            log.info('[SM-YJJ-STAGE] %s after_v130 n=%d removed_tail=%d err_keep=%d has600711=%s list=%s' % (
                context.current_dt.strftime('%Y%m%d %H:%M:%S'),
                len(g.yjj_candidates),
                removed_tail,
                removed_err,
    ...
    if g.yjj_candidates and g.market_mode == 'bull':
        ...
        if context.current_dt.strftime('%Y-%m-%d') == '2020-07-14':
            log.info('[SM-YJJ-STAGE] %s after_left n=%d has600711=%s list=%s' % (
                context.current_dt.strftime('%Y%m%d %H:%M:%S'),
                len(g.yjj_candidates),
                '600711.XSHG' in [_sm_code(x) for x in g.yjj_candidates],
                _sm_codes(g.yjj_candidates),
...
def _auction_yiqian_dynamic_value(context):
    ...
    active_for_auction = getattr(g, 'route_active', g.active)
    if active_for_auction == 'v227':
        if g.stoploss_cooldown > 0 and g.market_mode != 'bull':
            return 0.0
    elif active_for_auction == 'rzq+zb':
        if g.rzq_cooldown > 0:
            return 0.0
    else:
        return 0.0
