...
OUT = os.path.join(ROOT, "rebuild_2020_full_v16_probe")
...
def main():
    ...
    start = time.time()
    engine = Engine(strategy_code, "2020-01-01", "2020-12-31", 1000000, compat=EmotionGateJQCompat(ROOT))
    equity, trades, logs, metrics = engine.run()
    elapsed = time.time() - start

    equity.to_csv(os.path.join(OUT, "local_equity_2020_full.csv"), index=False)
    trades.to_csv(os.path.join(OUT, "local_trades_2020_full.csv"), index=False)
    with open(os.path.join(OUT, "local_run_2020_full.log"), "w", encoding="utf-8") as f:
        for line in logs:
            f.write(line + "\n")
