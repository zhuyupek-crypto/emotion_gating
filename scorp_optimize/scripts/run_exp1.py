...
# 实验一的低开区间配置
# A: -2% ~ -3%
# B: -3% ~ -4% (即当前主策略 strategy_v227_scorp.py)
# C: -4% ~ -5%
# D: -5% ~ -6%
EXP_CONFIGS = {
    "v227_scorp_exp1_A": ("-2% ~ -3%", "open_pct < -0.03 or open_pct > -0.02"),
    "v227_scorp_exp1_C": ("-4% ~ -5%", "open_pct < -0.05 or open_pct > -0.04"),
    "v227_scorp_exp1_D": ("-5% ~ -6%", "open_pct < -0.06 or open_pct > -0.05"),
}
...
def generate_exp_strategies():
    ...
    # 替换目标文本：
    # 查找 open_pct < -0.04 or open_pct > -0.03
    target_str = "open_pct < -0.04 or open_pct > -0.03"
    if target_str not in code:
        print("[ERROR] Could not find open_pct filter string in strategy_v227_scorp.py!")
        sys.exit(1)
...
def analyze_results_metrics():
    print("\n=================== Experiment 1 Analysis ===================", flush=True)
    out_base = os.path.join(ROOT, "bare_runs_analysis", "runs")
    
    # 包含 baseline (v227_scorp) 和其它 3 个实验策略
    ...
    # 写入报告
    report_path = os.path.join(ROOT, "bare_runs_analysis", "exp1_report.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# 实验一：低开区间扩容评估报告\n\n")
        f.write(df_res.to_markdown(index=False) + "\n")
    print(f"Generated Experiment 1 report at {report_path}")
