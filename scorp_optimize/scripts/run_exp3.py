...
# 实验三：仓位加权测试
# 替换 strategy_v227_scorp.py 中的 scale = 1.0
EXP_CONFIGS = {
    "v227_scorp_exp3_p12": ("1.2倍仓位", "scale = 1.2"),
    "v227_scorp_exp3_p15": ("1.5倍仓位", "scale = 1.5"),
    "v227_scorp_exp3_p20": ("2.0倍仓位", "scale = 2.0"),
    "v227_scorp_exp3_pdyn": ("动态 bear 强度加仓", 
                             "idx = attribute_history(g.idx_code, 60, '1d', ['close'])['close']\n    ma60 = np.mean(idx)\n    price = idx.iloc[-1]\n    bear_strength = max(0.0, (ma60 - price) / ma60)\n    scale = 1.0 + bear_strength * 4.0"),
}
...
def generate_exp_strategies():
    ...
    # 替换目标文本：
    target_str = "    scale = 1.0"
    if target_str not in code:
        print("[ERROR] Could not find scale = 1.0 string in strategy_v227_scorp.py!")
        sys.exit(1)

    generated_paths = {}
    for name, (label, replacement) in EXP_CONFIGS.items():
        exp_code = code.replace(target_str, f"    {replacement}")
        path = os.path.join(ROOT, "bare_runs_analysis", "strategies", f"{name}.py")
        with open(path, "w", encoding="utf-8") as f:
            f.write(exp_code)
        print(f"Generated strategy file for {label}: {path}")
...
def analyze_results_metrics():
    print("\n=================== Experiment 3 Analysis ===================", flush=True)
    out_base = os.path.join(ROOT, "bare_runs_analysis", "runs")
    
    # 包含 baseline (Slots=2, scale=1.0) 和其它加仓策略
    strategies_to_analyze = {
        "v227_scorp": "1.0倍仓位 (Baseline)",
        "v227_scorp_exp3_p12": "1.2倍仓位",
        "v227_scorp_exp3_p15": "1.5倍仓位",
        "v227_scorp_exp3_p20": "2.0倍仓位",
        "v227_scorp_exp3_pdyn": "动态 bear 强度加仓",
    }
    ...
    report_path = os.path.join(ROOT, "bare_runs_analysis", "exp3_report.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# 实验三：仓位加权与风险预算评估报告\n\n")
        f.write(df_res.to_markdown(index=False) + "\n")
    print(f"Generated Experiment 3 report at {report_path}")
