...
def initialize(context):
    ...
    g.v227_stop = -0.05
    g.yjj_candidates = []; g.bear_candidates = []; g.yjj_yclose = {}
    g.market_mode = 'bear'; g.raw_market_mode = 'bear'; g.last_prepare_year = None; g.bull_sticky = 0
    g.prev_first_boards = []; g.first_board_perf = 0.0; g.board_heights = []; g.stoploss_cooldown = 0
    g.owner = {}; g.buy_mode = {}; g.leader_holds = {}; g.leader_candidates_for_tag = []; g.dieting = []
    g.bull_consec_loss = 0; g.bull_cooldown = 0; g.non_bull_consec_wins = 0
    g.bull_force_clear = False; g.bull_release_confirm_pending = False; g.bull_release_guard = False
    ...
    g.info = None
    g.active = 'v227'
    g._today_max_boards = 0
    g.low_price_factor_enabled = True; g.low_price_ref = 20.0; g.low_price_weight = 0.15; g.low_price_min_win_rate = 0.45
    g.v227_shock_cooldown_enabled = True; g.v227_shock_loss_threshold = -0.048; g.v227_shock_cooldown_days = 1
    g.v227_shock_cooldown = 0; g.prev_portfolio_value = None
    g.recent_trades = deque(maxlen=WIN_WINDOW); g.recent_core_trades = deque(maxlen=WIN_WINDOW)
    g.fb_pct = 0.5; g.fb_perf_history = deque(maxlen=FB_WINDOW)
