...
# ==============================================================================
# DIAG 补丁：静默母版自身 log，只放行 [DIAG 前缀 + 关键成交日志
# 注：此补丁仅在 globals() 已有 log 对象后生效；聚宽运行时 log 是注入的全局
# ==============================================================================
_DIAG_INSTALLED = [False]

def _install_diag_log_filter():
    if _DIAG_INSTALLED[0]:
        return  # 幂等：已安装过则跳过，避免捕获自身导致递归
    try:
        _real_info = log.info
    except (NameError, AttributeError):
        return  # log 还没注入，等运行期再说

    _ALLOW = ('[DIAG', '[v227买', '[天蝎', '[scorpion', '[v227卖')

    def _silent_info(msg, *args, **kwargs):
        try:
            s = msg if not args else (msg % args)
        except Exception:
            s = str(msg)
        if isinstance(s, str) and any(s.startswith(p) for p in _ALLOW):
            _real_info(msg, *args, **kwargs)

    def _noop(*a, **kw):
        pass

    log.info = _silent_info
    if hasattr(log, 'warn'):    log.warn = _noop
    if hasattr(log, 'warning'): log.warning = _noop
    if hasattr(log, 'debug'):   log.debug = _noop
    # log.error 不拦截
...
_DIAG_LOG = log.info  # 此时 log.info 已是 _silent_info，但它对 [DIAG 是放行的，直接用即可

def _dlog(msg):
    """DIAG 行写日志区。所有 DIAG 行必须以 [DIAG 开头才会被放行。"""
    log.info(msg)
